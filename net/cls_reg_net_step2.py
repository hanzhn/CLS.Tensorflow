...
def conv_with_bn_relu(inputs, filters, kernel_size, strides, training, data_format):
  inputs=conv2d_fixed_padding(inputs, filters=filters,
            kernel_size=kernel_size, strides=strides, data_format=data_format)
  inputs = batch_norm(inputs, training, data_format)
  inputs = tf.nn.relu(inputs)
  return tf.identity(inputs)

def deconv_with_bn_relu(inputs, filters, kernel_size, strides, training, data_format):
  inputs = tf.layers.conv2d_transpose(inputs, filters=filters,
            kernel_size=kernel_size, strides=strides, data_format=data_format, padding='same')
  inputs = batch_norm(inputs, training, data_format)
  inputs = tf.nn.relu(inputs)
  return tf.identity(inputs)

def hour_down_blocks(inputs, output_size, training, data_format):
  inputs = conv_with_bn_relu(inputs, filters=output_size/2, kernel_size=1, strides=1, training=training, data_format=data_format)
  inputs = conv_with_bn_relu(inputs, filters=output_size/2, kernel_size=3, strides=1, training=training, data_format=data_format)
  for_add = tf.identity(inputs)
  inputs = conv_with_bn_relu(inputs, filters=output_size, kernel_size=3, strides=2, training=training, data_format=data_format)
  return tf.identity(inputs), for_add

def hour_up_blocks(inputs, output_size, training, data_format):
  inputs = conv_with_bn_relu(inputs, filters=output_size*2, kernel_size=1, strides=1, training=training, data_format=data_format)
  inputs = conv_with_bn_relu(inputs, filters=output_size*2, kernel_size=3, strides=1, training=training, data_format=data_format)
  inputs = deconv_with_bn_relu(inputs, filters=int(output_size), kernel_size=3, strides=2, training=training, data_format=data_format)
  return tf.identity(inputs)
...
def regression_points(top_feature, lower_feature, point_num, training, data_format):
  ...
  with tf.variable_scope('second_loc'):
    ...
    if training:
      second_inputs = tf.identity(lower_feature)
    else:
      ...
      crops = tf.concat(location_list, axis=0)
      if data_format == 'channels_first':
        # Convert the feature_map from channels_first (NCHW) back to channels_last (NHWC) 
        # for tf.image.crop_and_resize.
        crops = tf.transpose(crops, [0, 3, 1, 2])
      second_inputs = crops
    add_features = []
    output_size = 32
    for i in range(4):
      with tf.variable_scope('up{}'.format(i)):
        output_size *= 2
        second_inputs, for_add = hour_down_blocks(second_inputs, output_size, training, data_format)
        add_features.append(tf.identity(for_add))

    for i in range(3, -1, -1):
      with tf.variable_scope('down{}'.format(i)):
        output_size /= 2
        second_inputs = hour_up_blocks(second_inputs, output_size, training, data_format)
        second_inputs = second_inputs + add_features[i]

    with tf.variable_scope('after'):
      second_inputs = conv_with_bn_relu(second_inputs, filters=64, kernel_size=3, strides=1, 
                                  training=training, data_format=data_format)
      second_inputs = conv_with_bn_relu(second_inputs, filters=64, kernel_size=3, strides=1, 
                                  training=training, data_format=data_format)
      second_inputs = conv_with_bn_relu(second_inputs, filters=33, kernel_size=1, strides=1, 
                                  training=training, data_format=data_format)
      location_map = tf.identity(second_inputs)

    with tf.variable_scope('DSNT'):
      if data_format == 'channels_first':
        location_map = tf.transpose(location_map, [0, 2, 3, 1])
      inputs_shape_hw = int(64)
      kernel = np.arange(inputs_shape_hw)
      kernel = kernel / (inputs_shape_hw-1.)
      ...
      location = tf.reshape(location_map, [-1, inputs_shape_hw**2, 33])
      location = tf.nn.softmax(location, 1)
      # print(X, inputs)
      xs = tf.reduce_sum(X*location, 1, keepdims=True) #B*1*C
      ys = tf.reduce_sum(Y*location, 1, keepdims=True) #B*1*C
      location = tf.concat([xs,ys], 1)                #B*2*C
      location_stop = tf.stop_gradient(location)
      location_stop = tf.reshape(location_stop, [-1,2*33])
      location_c = tf.layers.dense(inputs=location_stop, units=2)
      location_c = tf.identity(location_c, 'center_dense')
      print(location_c)
  return loc_dence, location, location_c
...
class Model(object):
  ...
  def __call__(self, inputs, training):
    ...
    with self._model_variable_scope():
      ...
      orig_inputs = tf.identity(inputs)
      if training:
        inputs = tf.zeros([1,3,224,224])
      inputs = conv2d_fixed_padding(
    ...
    if self.is_attention and self.is_regression:
      with tf.variable_scope('REG'):
        ...
        loc, location, location_c = regression_points(self.attentioned_feature, orig_inputs, 2, training, self.data_format)
      return final_cls_dense, loc, location, location_c
    # If don't regress.
