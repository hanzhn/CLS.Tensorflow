...
tf.app.flags.DEFINE_boolean(
    'cls_block', False,
    'Use cls block or not. True/False')
tf.app.flags.DEFINE_boolean(
...
def input_pipeline(dataset_pattern='train-*', is_training=True, batch_size=FLAGS.batch_size):
    def input_fn():
        ...
        image_preprocessing_fn = lambda image_, label_: cls_preprocessing.preprocess_image(image_, label_, target_shape, 
                                                                    ...
                                                                    output_rgb=False)
        if FLAGS.regression_block and FLAGS.cls_block:
            ...
            exit('Can not make cls and reg both False!')
        ...
        with tf.variable_scope('losses'):
            ...
            if FLAGS.regression_block:
                ...
                with tf.variable_scope('reg_loss'):
                    ...
                    print(loc_targets)
                    loc_loss1 = tf.losses.absolute_difference(labels=tf.reshape(loc_targets,[-1,36]),
                                                            predictions=tf.reshape(loc,[-1,36]), 
